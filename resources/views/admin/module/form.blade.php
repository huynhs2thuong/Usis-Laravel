<div class="primary col s9">
    ...
    <div class="input-field">
        @include('partials.lang_input', ['type' => 'text', 'model' => 'module', 'attr' => 'canonical', 'title' => trans('admin.field.canonical')])
    </div>
	<div class="input-field">
	    @include('partials.lang_input', ['type' => 'textarea', 'attr' => "meta_title", 'model' => 'module', 'class' => ' materialize-textarea', 'title' => trans('admin.field.meta_title')])
	</div>
    <div class="input-field">
        @include('partials.lang_input', ['type' => 'textarea', 'attr' => "meta_desc", 'model' => 'module', 'class' => ' materialize-textarea', 'title' => trans('admin.field.meta_desc')])
    </div>
    <div class="input-field">
        @include('partials.lang_input', ['type' => 'textarea', 'attr' => "meta_keyword", 'model' => 'module', 'class' => ' materialize-textarea', 'title' => trans('admin.field.meta_keyword')])
    </div>
</div>
<div class="side col s3">
    ...
    <div class="cat-middle card hoverable">
        ...
        <div class="divider"></div>
        <p>
            {{ Form::radio('parent_id',0, true, ['id' => 'category-none']) }}
            <label for="category-none">None</label>
        </p>
        @foreach ($cates as $k => $cate)
            <p>
                @if(intval($module->parent_id) == intval($k))
                {{ Form::radio('parent_id', $k, true, ['id' => 'category-' . $k]) }}
                @else
                    {{ Form::radio('parent_id', $k, false, ['id' => 'category-' . $k]) }}
                @endif
	...
	<div class="cat-bottom card hoverable">
        ...
        <div class="postimagediv">
            @if(isset($module->resource_id))
            <img src="{{ $module->getimageThumb() }}" class="responsive-img" alt="">
            @endif
            <input type="hidden" name="resource_id" value="{{ $module->resource_id }}">
            <div class="clearfix"></div>
            <a class="btn waves-effect waves-light cyan {{ $module->resource_id ? 'hide' : '' }} set-image modal-trigger" href="#modal-upload">@lang('admin.button.set image')</a>
            <a class="btn waves-effect waves-light cyan {{ $module->resource_id ? '' : 'hide' }} remove-image">@lang('admin.button.remove image')</a>
        </div>
    </div>
</div>
