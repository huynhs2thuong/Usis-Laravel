...
<section class="u004 row-section bg-white wow fadeInUpS">
	<div class="container">
		{!!$page->description!!}
	</div>	
</section>
...
<section class="row-section list12">
	...
	<div class="section-header wow fadeInUpS" >
		<h2 class="section-title text-center"> <a href="{{route('subpagedv',['slug'=>$services[0]->slug])}}">@lang('menu.page.service')</a> </h2>
		</div>
		<div class=" list slide3-arr owl-carousel">
		@foreach($dautudc as $key => $dautu)
			<a href="{{action('SiteController@showPageDichVu',['slug'=>$dautu->getSlug()])}}" class="list_item wow fadeInUpS">
		...
		<!-- FIRST for slide -->
			...
			@foreach($event as $key => $ev)
				<div class="item <?php if($key == 0) echo 'active'; ?> "  title="{{$ev->title}}">
					<figure class="snip1544" style="overflow: unset;">
