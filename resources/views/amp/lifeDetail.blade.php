@extends('layouts.amp')
@push('styleinline')
.container .widget-content ul{padding-left:0}
@endpush
	...
	<div class="container"> 
		...
		<div class=" list slide3-arr owl-carousel top-arrows">
			@foreach($relateds as $item)
				<div class="list_item ">
					<a href="{{action('AmpController@life',$item->slug)}}" class="list_img thumbCover_60">

					@if($item->resource_id !=NULL)
						<amp-img src="{{$item->imageUrl($item->getImage('thumbnail'),360,216,100)}}" width="360" height="216" layout="responsive" alt="" /> 
					@else
						@if($item->content_image != '')
							<?php 
								$urlimage = 'https://www.usis.us/uploads/images/contents/'.$item->content_image;
							?>
							<amp-img src="{{$item->imageUrl($urlimage,360,216,100)}}" width="360" height="216" layout="responsive" alt="" /> 
						@else
							<amp-img src="{{$item->imageUrl($item->getImage('thumbnail'),360,216,100)}}" width="360" height="216" layout="responsive" alt="" /> 
						@endif
					@endif
					...
					<div class="list_text">
						...
						<a href="{{action('AmpController@life',$item->slug)}}">
