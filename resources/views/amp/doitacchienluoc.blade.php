@extends('layouts.amp')
@push('styleinline')
h1{margin-bottom:0;padding-bottom:20px}.h1, h1 {font-size: 28px;line-height: 38px;text-transform:uppercase}
@endpush
		...
		<div id="breadcrumbs">
		    ...
		    <ul class="breadcrumb">
		      <li><a href="{{ ($current_locale == 'vi') ? '/amp' : '/en/amp' }}">@lang('menu.page.home') </a></li>
				...
				?>
					<a href="{{action('AmpController@showPage',$slugpage)}}">{{$secPage->title}}</a>
				</li>
				@endif
		...
		<div class="slide3-a-arr owl-carousel">
			...
			@foreach($relateds as $team)
				<div class="item text-center">
					<div class="img "><a href="{{route('dtAncuChitietamp',['slug'=>$slug,'permalink'=>$permalink,'partners'=>$team->slug])}}">
						<amp-img src="{{$team->image}}" alt="{{$team->title}}" width"512" height="400" layout="responsive" /></a></div>
...
@include('partials.canonical')
	...
	<script type="application/ld+json">
	{
	    ...
	    "itemListElement": [{
	      ...
	      "position": 2,
	      "item": {
	        "@id": "{{action('AmpController@showPage',$slugpage)}}",
	        "name": "{{$secPage->title}}"
	      }
	    },{
	      "@type": "ListItem",
