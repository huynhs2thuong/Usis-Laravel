...
<section class="top-page bg-white">
		<div id="breadcrumbs">
		    ...
		    <ul class="breadcrumb">
		      ...
		      @endif
		      <li><a href="{{action('SiteController@showPageDichVu',['slug'=>$slug])}}">{{$category->title}}</a></li>
		    </ul>
		</div>
		</div>
	<div class="container"> 
		<div class="page-title wow">
				<h1 class="title"> <span>@if($category){{$category->title}}@else @lang('menu.page.service') @endif</span></h1>
		</div>
	</div>	
</section>
	...
	<div class="container"> 
		...
		<div class="menu-content">
			<ul>
				...
				<li <?php if($cate->id == $category->id): ?> class="active" <?php endif; ?>>
					<a href="{{ action('SiteController@showPageDichVu',['slug'=>$cate->slug]) }}">{{ $cate->title }}</a>
				</li>
				@endif
				@endforeach
				@if($lifemodule)
		...
		<div class=" list row">
			@if(isset($checkPag) && $checkPag == TRUE)
				...
				@endforeach
			@else
				@foreach(collect($pages)->paginate( 12 ) as $data)

				<div class="col-md-4 col-sm-6 col-xs-6 col-xxs-12">
					<div class="list_item  wow">
						<a href="{{action('SiteController@showPageDichVu',$data->slug)}}" class="list_img thumbCover_60">
						@if($data->resource_id != NULL)
							<img class="img-lazy" data-src="{{$data->imageUrl($data->getImage('thumbnail'),358,215,100)}}" alt="" /> 
						@else
							@if($data->content_image != '')
						...
						<div class="list_text">
							<a href="{{action('SiteController@showPageDichVu',$data->slug)}}" class="list_title">
								{{ stripslashes($data->title) }}
							</a> 
							<div class="list_position">{!! stripslashes($data->excerpt) !!}</div>
						</div>
